...
loop(DB, Pid_response) ->
    receive
        {insert, {Sock, Tablename, Client_id, Client_ip, Client_port, Client_uploaded, Client_downloaded, Client_left, Torrent_hash}} ->
            ...
            Pid_response ! {Sock, db_select_peers(DB, Tablename, Torrent_hash, Client_id)},
            loop(DB, Pid_response)
    end.
...
create_peers_list([{Peer_id, Peer_ip, Peer_port} | TailDict], []) ->
    create_peers_list(TailDict, lists:append([], [{dict, dict:from_list([{<<"peer_id">>, Peer_id},{<<"ip">>, list_to_binary(string:strip(binary_to_list(Peer_ip)))}, {<<"port">>, Peer_port}])}]));
create_peers_list([{Peer_id, Peer_ip, Peer_port} | TailDict], Acc) ->
    create_peers_list(TailDict, lists:append(Acc, [{dict, dict:from_list([{<<"peer_id">>, Peer_id},{<<"ip">>, list_to_binary(string:strip(binary_to_list(Peer_ip)))}, {<<"port">>, Peer_port}])}]));
