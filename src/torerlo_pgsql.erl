...
db_create(DB, Table_torrent, Table_peers, Table_seeds) ->
    pgsql:equery(DB, string:concat(string:concat("CREATE TABLE ", Table_torrent), " (Name char(20), Link char(100), Owner char(20))")),
...
db_insert(DB, Tablename, Name_torrent, Link_torrent, Owner_torrent) ->
    Query = string:concat(string:concat("INSERT INTO ", Tablename), " VALUES ($1, $2, $3);"),
    pgsql:equery(DB, Query, [Name_torrent,Link_torrent,Owner_torrent]).
